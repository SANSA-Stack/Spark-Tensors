...
/**
  ...
  * @author Yizhi Liu
  */
class MyPointIter private[mxnet](
                                ...
                                private val ctx: Context) extends DataIter {
  ...
  override def next(): DataBatch = {
    ...
    if (index >= 0 && index < cache.size) {
      ...
      val dataBuilder = NDArray.empty(dataShape, ctx)
      val labelBuilder = NDArray.empty(Shape(_batchSize), ctx)
      var instNum = 0
