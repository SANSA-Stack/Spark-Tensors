...
/**
  ...
  */
class TransE(numEntities: Int, numRelations: Int, latentFactors: Int, batchSize: Int, literalsEnabled: Boolean = false) {
  class Model(opt: Optimizer,
    ...
    val module = new Module(trainSymbol, contexts = Array(ctx))
...
//      s.Embedding()()(Map("data" -> s.BlockGrad()()(Map("data" -> data)), "weight" -> literalEntityWeight, "input_dim" -> numEntities, "output_dim" -> latentFactors))
    ...
    def relationEmbedding(data: Symbol) =
      s.Embedding("relations")()(Map("data" -> data, "weight" -> relationWeight, "input_dim" -> numRelations, "output_dim" -> Math.pow(newWidth, 2).toInt))
...
//        val finalData = finalInputStuff zip finalLiteralAll flatMap { case (a, b) => Seq(a, b) }
        ...
        val opt = new AdaGrad(learningRate = 0.01f, wd = 0.001f)
...
//        val literalTrainer = new Model(opt, literalModel, scoreModel, literalData, paramNames, ctx, "literaldata", labels = Some(literalLabels))
        for (iter <- 0 until size * 1) {
          if(!dataIter.hasNext || (iter > 0 && iter % size == 0)) {
            println("Epoch end!")
            inputStuff = Random.shuffle(inputStuff)
...
//    println(hits.mrr)
    ...
    val myIter = new MyPointIter(vectors2.toIterator, Shape(5), 100, "data", "label", ctx2)
...
//    println(predictions.map(x => x.toArray.mkString("|")).mkString("\n\n"))
    ...
    val numEntities2 = numEntities + (100 - numEntities % 100)

    val hitsTest = new EvalMetrics(model2, 0, 100,
    ...
    val hitsTest2 = new EvalMetrics(model2, 2, 100,
      NDArray.array(Array.range(0,numEntities2).map(_.toFloat), Shape(1,numEntities2), ctx2),
      myIter, ctx2)
    //    println(hits2.hits(5,10).mkString(","))
    println(hitsTest2.hits(10))

    val hitsTrain = new EvalMetrics(model2, 0, 100,
      NDArray.array(Array.range(0,numEntities2).map(_.toFloat), Shape(1,numEntities2), ctx2),
      new MyPointIter(vectors.toIterator, Shape(5), 100, "data", "label", ctx2), ctx2)
